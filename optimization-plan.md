...
- ✅ `qcom/a630_sqe.fw`(34K) + `qcom/a640_gmu.bin`(37K)：Adreno 通用微码（确认缺失 → 修 GPU 故障）。
  注：原列表的 `a630_gmu.bin` 是 sdm845 的 GMU，对 sm8150/Adreno 640 无用，已删；正确的是 `a640_gmu.bin`
- ✅ `qca/crbtfw21.tlv`(173K)：QCA 蓝牙固件（确认缺失 → 修 BT 不起，实机已确认 hci0 注册）
- 🔬 `qcom/sm8150/a640_zap.mbn`(14K)：GPU zap shader。raphael DTB 把 firmware-name 指到设备专属路径
  `qcom/sm8150/{xiaomi,Xiaomi}/raphael/a640_zap.mbn`（下游内核用大写 `Xiaomi`），故 `09` 把通用 zap
  额外拷到这两个路径（否则 dmesg 报 `Unable to load …a640_zap.mbn → gpu hw init failed -2`）。
  若通用签名被 TZ 拒绝（`-22 initializing firmware`），改从厂商 vendor 分区 / pmOS adreno 包提取设备专用 zap
